...
def fetchIngredientsFromGPT(text):
    prompt = f"""Please extract ingredients from the text, 
    make sure to translate ingredients in english for example bangun should be eggplant, 
    also if possible extract quantity in array form like ['egg-1', 'tomato-2'],
    in quantity please include only integer like [egg-1] here egg is ingredient and 1 is quantity,
    make sure to not add any additional text in response just array,
    if unable to extract quantity keep it 0 and if the quantity is greater than 10 keep it 1,
    in case no ingredients available please provide empty array like this []
    Keep the first letter capital, text is: {text}"""
